...
@section('content')
    ...
    <div class="row justify-content-center">
        <div class="col-md-6">
            <form class="card shadow mb-5" action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select id="features" name="features" class="form-control @error('features') is-invalid @enderror">
                            @foreach(trans('feature') as $key => $feature)
                                <option value="{{ $key }}" {{ (old('features') == $key) ? 'selected' : '' }}>{{ $feature }}</option>
                            @endforeach
                        </select>
                        @error('features')
